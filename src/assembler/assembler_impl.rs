...
impl Assembler {
	...
	pub fn assemble(&mut self, file_name: &str) -> Result<(), Error> {
		self.expr.init(&mut self.labels, &mut self.constants, &mut self.forward_references, &mut self.line_number, &mut self.file_name);
	...
	pub(crate) fn curr_line(&self) -> isize {
	...
	pub fn second_pass(&mut self) -> Result<(), Error> {
		while let Some(mut fwd_ref) = self.forward_references.pop() {
			...
			if fwd_ref.is_expression {
				...
				data = match self.get_label_or_constant_value(fwd_ref.label.as_str()) {
					Ok(n) => n,
					Err(e) => return Err(self.error_second_pass(SyntaxError, fwd_ref.line_no, &fwd_ref.file_name)),
				}
			}
	...
	pub fn error(&mut self, t: ErrorType) -> Error {
		Error {
			line_no: self.curr_line(),
			message: t.to_string(),
			level: ErrorLevel::Fatal,
			file_name: self.cur_file(),
		}
	}
	...
	pub(crate) fn try_resolve_label(&mut self, name: &str, pc_offset: isize, relative: bool) -> u16 {
		...
		if let Some(a) = self.constants.get(label_name) {
			...
			self.forward_references.push(ForwardReference {
				...
				file_name: self.file_name.last().unwrap_or(&String::from("<Unknown>")).clone(),
			});
		}
		return addr;
	}
