...
use crate::assembler::{Assembler, Error, ErrorLevel, TokenReader, ForwardReference, AssemblerContext};
...
impl Assembler {
    ...
    pub fn assemble(&mut self, file_name: &str) -> Result<(), Error> {
        //self.expr.init(&mut self.context);
    ...
    pub(crate) fn first_pass(&mut self, file_name: &str) -> Result<(), Error> {
        ...
        self.context.file_name.pop();
        self.context.line_number.pop();
        Ok(())
    }

    pub fn second_pass(&mut self) -> Result<(), Error> {
        while let Some(mut fwd_ref) = self.context.forward_references.pop() {
            let mut data: isize;
    ...
    pub fn relative(&mut self) -> Result<u8, Error> {
        match self.next_token()? {
            Number(n) => Ok((n - (self.context.current_pc as isize + 2)) as u8),
            ConstLabel(s) => {
                let mut addr = self.try_resolve_label(&s, 1, true) as isize;
                let pc = (self.context.current_pc + 2) as isize;
                if addr == 0 {
                    addr = pc;
                }
    ...
    pub(crate) fn try_resolve_label(&mut self, name: &str, pc_offset: isize, relative: bool) -> u16 {
        ...
        if let Some(a) = self.context.constants.get(label_name) {
            addr = *a as u16;
        } else if let Some(a) = self.context.labels.get(label_name) {
            addr = *a as u16;
        } else {
            self.context.forward_references.push(ForwardReference {
                is_expression: false,
                pc: self.context.current_pc + pc_offset,
                label: label_name.to_string(),
                expression: vec![],
                is_relative: relative,
                byte_count: 2,
                line_no: self.context.line_number.last().unwrap_or(&0isize).clone(),
                file_name: self.context.file_name.last().unwrap_or(&String::from("<Unknown>")).clone(),
            });
        }
        return addr;
    }
    ...
    pub(crate) fn decode_number(&mut self, token: &Token, pc_offset: isize) -> Result<Option<isize>, Error> {
        match &token {
            ...
            ConstLabel(l) => if let Some(n) = self.context.constants.get(l) {
                Ok(Some(*n))
            } else {
    ...
    pub(crate) fn emit(&mut self, mut b: Vec<u8>) {
        if self.context.current_pc + b.len() as isize > 65535 {
            self.warn(ErrorType::PCOverflow)
        }
        self.context.current_pc += b.len() as isize;
    ...
    pub fn translate(&mut self, tokens: &mut Vec<Token>) -> Result<(), Error> {
        let len = self.context.line_number.len() - 1;
        self.context.line_number[len] += 1;
